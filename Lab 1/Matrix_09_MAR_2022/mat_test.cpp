...
void Mat_Test::mul_test() {
    ...
    // Test cases
    if (prod.get_rows() == r1 && prod.get_rows() == prod.get_cols()) { cout << "Test I: passed.\n"; }
    else { cout << "Test I: failed.\n"; }
    if (status) { cout << "Test II: passed.\n"; }
    else { cout << "Test II: failed.\n"; }
}
...
void Mat_Test::transpose_test() {
    ...
    if (mt_r == m_r.get_cols() &&
        mt_c == m_r.get_rows()) { cout << "Test I: passed.\n"; }
    else { cout << "Test I: failed.\n"; }// Test II
    if (status) { cout << "Test II: passed.\n"; }
    else { cout << "Test II: failed.\n"; }
}
...
void Mat_Test::bool_test() {
    ...
    // 1. Identity
    if (m_i.isIdentity() && m_i.get_rows() == r) { cout << "1. isIdentity():\t passed.\n"; }
    else { cout << "1. isIdentity():\t failed.\n"; }
    // 2. Idempotent
    Matrix m1(r1, c1, 3);
    Matrix m_sym = m1 * m1.transpose();
    if (!m_i.isIdempotent())cout << "!m_i.isIdempotent()";
    if (m_sym.isIdempotent())cout << "m_sym.isIdempotent()";
    if (m_i.isIdempotent() && !m_sym.isIdempotent() && m_i.get_rows() == r) {
        cout << "2. isIdempotent():\t passed.\n";
    } else { cout << "2. isIdempotent():\t failed.\n"; }
    // 3. Square
    if (m_i.isSquare() && !m_r1.isSquare() && m_i.get_rows() == r) { cout << "3. isSquare():\t\t passed.\n"; }
    else { cout << "3. isSquare():\t\t failed.\n"; }
    // 4. Symmetric
    Matrix m_asym = m_sym;
    // min_count = 3
    m_asym.access(1, 2) -= 1;
    ...
    } else { cout << "4. isSymmetric():\t failed.\n"; }
    // Triangular Matrices
    ...
    for (int i = 0; i < ns_r; i++) {
        for (int j = 0; j < ns_c; j++) {
            if (j < i) { m_nsu.access(i, j) = 0; }
            if (j > i) { m_nsl.access(i, j) = 0; }
        }
    }
    // Test cases
    // 5. UpperTriangle
    if (m_i.isUpperTriangle() && m_ut.isUpperTriangle() &&
        !m_nsu.isUpperTriangle() && m_i.get_rows() == r) { cout << "5. isUpperTriangle():\t passed.\n"; }
    else { cout << "5. isUpperTriangle():\t failed.\n"; }
    // 6. LowerTriangle
    if (m_i.isLowerTriangle() && m_lt.isLowerTriangle() &&
        !m_nsl.isLowerTriangle() && m_i.get_rows() == r) { cout << "6. isLowerTriangle():\t passed.\n"; }
    else { cout << "6. isLowerTriangle():\t failed.\n"; }
}
...
void Mat_Test::fill_test() {
    ...
    // Test cases
    if (status && m.get_rows() == r1) { cout << "Test I: passed.\n"; }
    else { cout << "Test I: failed.\n"; }
}
...
void Mat_Test::scale_test() {
    ...
    // Test cases
    if (status && m_i.get_rows() == r) { cout << "Test I:\t\t\t\t passed.\n"; }
    else { cout << "Test I:\t\t\t\t failed.\n"; }
    if (res2 == m_i && m_i.get_rows() == r) { cout << "Test II:\t\t\t passed.\n"; }
    else { cout << "Test II:\t\t\t failed.\n"; }
    if ((m_test.get_rows() && m_test.get_cols() && isinf(m_test.access(0, 0)))
        || m_i.get_rows() != r) { cout << "Test III (div. by zero):\t failed.\n"; }
    else { cout << "Test III (div. by zero):\t passed.\n"; }
}
...
void Mat_Test::minmax_test() {
    ...
    // Test cases
    if (m_minmax.minElement() == min_val) { cout << "Test I: passed.\n"; }
    else { cout << "Test I: failed.\n"; }
    if (m_minmax.maxElement() == max_val) { cout << "Test II: passed.\n"; }
    else { cout << "Test II: failed.\n"; }
}
